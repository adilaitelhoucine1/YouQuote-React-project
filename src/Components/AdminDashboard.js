...
// Form modal component - fixed to always show input field
...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState('categories'); // Change default to categories
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Only fetch tags and categories
  ...
  const handleDelete = async (type, id) => {
    ...
    try {
      ...
      setError(`Failed to delete ${type === 'tags' ? 'tag' : 'category'}. Please try again.`);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Create payload with just the name property
      ...
      <header className="bg-white shadow-md py-4 px-6">
        <div className="container mx-auto flex justify-between items-center">
          <div className="flex items-center">
            <h1 className="text-2xl font-bold text-purple-600">YouQuote Admin</h1>
      ...
      <main className="container mx-auto py-8 px-4">
        ...
        {activeTab === 'tags' && (
          <ItemList 
            items={tags}
            type="tags"
            onEdit={(type, item) => openModal('editTag', item)}
        ...
        {activeTab === 'categories' && (
          <ItemList 
            items={categories}
            type="categories"
            onEdit={(type, item) => openModal('editCategory', item)}
            onDelete={handleDelete}
